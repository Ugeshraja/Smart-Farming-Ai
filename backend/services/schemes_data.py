"""
SmartFarm AI - Government Agriculture Schemes Service & Dataset
Verified national and state agricultural schemes with official portals (.gov.in / .nic.in)
...
def evaluate_eligibility(
    ...
    """
    Deterministic rule-based eligibility evaluation comparing field profile
    parameters (crop, state, land size, etc.) against scheme rules.
    Does NOT use generative AI or make legal claims.
    """
    ...
    crop = (field_profile.get("crop_type") or "").strip().lower()
    location = (field_profile.get("field_location") or "").strip().lower()
    
    # Calculate acreage
    try:
        raw_size = float(field_profile.get("field_size", 2.0))
    except (ValueError, TypeError):
        raw_size = 2.0
    unit = (field_profile.get("field_size_unit") or "Acre").strip().lower()
    # Convert hectares to acres for uniform evaluation (1 ha = 2.471 acres)
    acres = raw_size * 2.471 if "hectare" in unit else raw_size

    supported_states = [s.lower() for s in rules.get("supportedStates", ["all india"])]
    supported_crops = [c.lower() for c in rules.get("supportedCrops", ["all"])]
    min_acres = rules.get("minLandSize")
    max_acres = rules.get("maxLandSize")

    # 1. State Check
    state_match = "all india" in supported_states or any(st in location or location in st for st in supported_states)
    if not state_match and location:
        return {
            "status": "not_eligible",
            "code": "state_mismatch",
            "badge": "✕ Not Eligible",
            "message": f"This scheme is designated for {', '.join(rules.get('supportedStates', []))} and may not apply in your location ({field_profile.get('field_location')}).",
            "disclaimer": "Final eligibility is subject to verification by the government scheme authority."
        }

    # 2. Crop Check
    crop_match = "all" in supported_crops or any(c in crop or crop in c for c in supported_crops)
    if not crop_match and crop:
        return {
            "status": "not_eligible",
            "code": "crop_mismatch",
            "badge": "✕ Not Eligible",
            "message": f"This scheme focuses on {', '.join(rules.get('supportedCrops', []))} crops rather than {field_profile.get('crop_type')}.",
            "disclaimer": "Final eligibility is subject to verification by the government scheme authority."
        }
    ...
    if max_acres is not None and acres > max_acres:
        return {
            ...
            "code": "size_exceeded",
            "badge": "✕ Not Eligible",
            "message": f"This scheme is targeted for landholdings up to {max_acres} Acres (current field size: {acres:.1f} Acres).",
            "disclaimer": "Final eligibility is subject to verification by the government scheme authority."
        }
    ...
    if min_acres is not None and acres < min_acres:
        return {
            ...
            "code": "size_threshold",
            "badge": "? Eligibility Check Required",
            "message": f"Requires minimum landholding of {min_acres} Acres. Check local cluster guidelines on the official portal.",
            "disclaimer": "Final eligibility is subject to verification by the government scheme authority."
        }
